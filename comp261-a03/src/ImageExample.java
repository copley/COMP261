...
/**
 * A simple example of how to extend the GUI class. The converBitmapToImage method is particularly useful.
 */
public class ImageExample extends GUI {
	...
	protected BufferedImage render() {
		...
		for (int x = 0; x < CANVAS_WIDTH; x++) {
			for (int y = 0; y < CANVAS_HEIGHT; y++) {
				float hue = (float) Math.sin((x + Math.pow(y, shift)) / (CANVAS_WIDTH));
	...
	/**
	 * Converts a 2D array of Colors to a BufferedImage. Assumes that bitmap is indexed by column then row and has imageHeight rows and imageWidth
	 * columns. Note that image.setRGB requires x (col) and y (row) are given in that order.
	 */
