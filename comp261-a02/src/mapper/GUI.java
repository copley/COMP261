...
public abstract class GUI {
	...
	/**
	 * Is called when the mouse wheel is rolled
	 */
	 ...
	 */
	protected abstract void onLoad(File nodes, File roads, File segments, File polygons, File restrictions, File traffic);
	
	/**
	 * Called when the DEBUG MODE button is pressed. Utilised to display the quad node structure overlay
	 */
	protected abstract void onDebug();

	/**
	 * Called when find path function is activated
	 */
	protected abstract void onFindPaths();

	/**
	 * Called when articulation points function is activated
	 */
	protected abstract void onArtPts();
	
	/**
	 * Called when articulation points ITERATIVE function is activated
	 */
	protected abstract void onArtPtsIter();
	...
	protected static final int DEFAULT_DRAWING_HEIGHT = 600;
	protected static final int DEFAULT_DRAWING_WIDTH = 800;
	...
	private static final String TRAFFIC_FILENAME_FILENAME = "NZtrafficLightCoords.txt";
	...
	private void initialise() {
		...
		load.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				File nodes = null, roads = null, segments = null, polygons = null, restrictions = null, traffic = null;
				...
				if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
					...
					for (File f : files) {
						if (f.getName().equals(NODES_FILENAME)) {
							...
							restrictions = f;
						} else if (f.getName().equals(TRAFFIC_FILENAME_FILENAME)) {
							traffic = f;
						}
					}
					...
					if (nodes == null || roads == null || segments == null) {
						...
						onLoad(nodes, roads, segments, polygons, restrictions, traffic);
						redraw();
					}
				}
			}
		...
		JCheckBox path = new JCheckBox("Find Paths");
		...
		JButton artPts = new JButton("ART PTS RECURSIVE");
		...
		JButton artPtsIter = new JButton("ART PTS ITERATIVE");
		artPtsIter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				onArtPtsIter();
			}
		});
